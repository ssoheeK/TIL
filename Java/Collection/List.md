...
```java
 class ArrayList{ 
    public static void main(String[] args){
        ...
        // remove method
        linkedList.removeFirst(); //가장 앞 데이터 제거
        linkedList.removeLast();  //가장 뒤 데이터 제거
        linkedList.clear();
        
        // 값 검색
        linkedList.contains(6); //값 검색(return 타입:boolean)
        linkedList.indexOf(6);  //해당 값의 인덱스 반환, 없으면 -1
  }
}
```